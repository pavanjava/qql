...
class AsyncExecutor(Executor):
    ...
    async def _fetch_collection_info(self, name: str):
        try:
    ...
    async def _resolve_topology(self, name: str) -> CollectionTopology:
        info = await self._fetch_collection_info(name)
        if info is None:
    ...
    async def _ensure_collection(
        ...
        vector_name = explicit_vector or self._default_dense_vector_name()
        created_topology = CollectionTopology(
            exists=True,
            is_named_dense=True,
            dense_names=(vector_name,),
            dense_sizes=((vector_name, vector_size),),
        )
        async with self._creation_lock:
            current_topology = await self._resolve_topology(name)
            if current_topology.exists:
                return await self._ensure_collection(
                    name, vector_size, current_topology, explicit_vector
                )
    ...
    async def _execute_show_collection(self, node: ShowCollectionStmt) -> ExecutionResult:
        info = await self._fetch_collection_info(node.collection)
        if info is None:
            raise QQLRuntimeError(f"Collection '{node.collection}' does not exist")
        config = info.config
